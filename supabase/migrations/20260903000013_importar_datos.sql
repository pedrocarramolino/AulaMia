-- AulaMia · Fase 08 · Restaurar una copia de seguridad (JSON) del propio usuario
--
-- Borra todos los datos del usuario y los reemplaza por los del JSON exportado.
-- SECURITY DEFINER para poder desactivar temporalmente el trigger de resiembra
-- de horarios; el user_id de cada fila se fuerza al del usuario autenticado.
...
begin
  ...
  -- 2. Insertar (padre -> hijo), forzando user_id
  ...
  -- clase, primer paso sin la autorreferencia
  ...
  -- clase, segundo paso: enlaces de recuperación
...
revoke execute on function public.importar_datos(jsonb) from public;
